...
focus:outline-none focus:ring-2 focus:ring-white/30"
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <Toggle
          ...
          chechked={revisionMode}
          onChange={() => setRevisionMode(!revisionMode)}
        />
        <Toggle
          lable="Include Diagram"
          chechked={includeDiagram}
          onChange={() => setIncludeDiagram(!includeDiagram)}
        />
        <Toggle
          lable="Include Charts"
          chechked={includeChart}
      ...
      {loading && (
        <div className="mt-4 space-y-2">
          ...
          <p className="text-xs text-gray-400 text-center">
            This may take up 2-5 minutes. Please don't close or refresh the page
          </p>
        </div>
      )}
    </motion.div>
...
function Toggle({ lable, chechked, onChange }) {
  return (
    <div
...
rounded-full bg-white shadow-[0_5px_15px_rgba(0,0,0,0.5)]"
          style={{
            left: chechked ? "1.6rem" : "0.25rem",
          }}
        ></motion.div>
      </motion.div>
      <span
        className={`text-sm transition-colors ${
          chechked ? "text-green-300" : "text-gray-300"
        }`}
      >
        {lable}
      </span>
