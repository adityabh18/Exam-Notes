  ...
  const handlePaying = async (plan) => {
    try {
      ...
      const result = await axios.post(
        serverUrl + "/api/credits/buy-credits",
        {
...
function PricingCard({
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className={`relative cursor-pointer rounded-xl p-6 bg-white
      border transition
      ${
        isSelected
          ? "border-black"
    ...
    >
      ...
      {popular && !isSelected && (
        <span
          className="absolute top-4 right-4 text-xs px-2 py-1
